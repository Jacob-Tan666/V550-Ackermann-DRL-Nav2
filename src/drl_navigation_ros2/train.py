...
def _drain_latest_weight(weight_queue):
    """只保留队列中最新的一份权重，避免 Worker 使用过期权重。"""
...
def _push_latest_weights(weight_queues, actor_state_dict):
    """向所有 Worker 广播最新 actor 权重，每个队列只保留最新一份。"""
...
# ============================================================
# 评估函数（仅在主进程 Learner 中调用）
# ============================================================

def eval_fn(model, env, scenarios, epoch, max_steps):
    """Learner 侧集中评估，避免污染采样 Worker。"""
    print("..............................................", flush=True)
    ...
    for scenario in scenarios:
        ...
        while count < max_steps:
            ...
            if terminal:
                break
            action = model.get_action(state, False)  # 评估不加噪声
...
# ============================================================
# Worker 进程：仅负责与环境交互、采样 transition
# ============================================================

def env_worker(worker_id, args, transition_queue, episode_queue, weight_queue,
               global_step_counter, stop_event):
    """并行采样 Worker。保留 GPU 推理。"""
    # 限制每个进程 PyTorch 线程数，防止多 Worker 争抢 CPU
    ...
    # Worker 使用 GPU 推理（按用户要求保留）
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Worker 侧构建 TD3 时禁用 writer / 自动保存，避免冲突
    model = TD3(
    ...
    model.actor.eval()  # Worker 只推理，不训练
    ...
    while not stop_event.is_set():
        ...
        if state_dict is not None:
            # 把 CPU tensor 迁到本 Worker 所在的 device
            state_dict = {k: v.to(device) for k, v in state_dict.items()}
            model.actor.load_state_dict(state_dict)

        # 读取全局 step（主进程统一维护），决定 warm-up 还是策略采样
        ...
        try:
            ...
            # Learner 跟不上时，丢弃最旧的 transition 不影响 off-policy 收敛
            pass
...
# ============================================================
# Learner 主进程：训练 + 评估 + 广播权重
# ============================================================

def main():
    mp.set_start_method('spawn', force=True)  # 避免 CUDA 上下文污染
    ...
    args = {
        ...
        # 训练总步数上限（以 Learner 收到的 transition 计数）
    ...
    # 扩大 replay buffer，适配多 Worker 并行采样
    ...
    # Learner 自己维护一个独立的评估环境（独立 ROS_DOMAIN_ID）
    ...
    for i in range(num_workers):
        p = mp.Process(
            target=env_worker,
            args=(i, args, transition_queue, episode_queue, weight_queues[i],
                  global_step_counter, stop_event),
        )
        p.start()
        workers.append(p)
    ...
    try:
        while total_env_steps < args['max_total_steps']:
            ...
            if now - last_status_time >= status_every_sec:
                ...
                if model.writer is not None:
                    model.writer.add_scalar("train_status/recent_goal_rate", recent_metrics["goal_rate"], total_env_steps)
                    model.writer.add_scalar("train_status/recent_collision_rate", recent_metrics["collision_rate"], total_env_steps)
                    model.writer.add_scalar("train_status/recent_timeout_rate", recent_metrics["timeout_rate"], total_env_steps)
                    model.writer.add_scalar("train_status/recent_episode_reward", recent_metrics["avg_reward"], total_env_steps)
                    model.writer.add_scalar("train_status/recent_episode_steps", recent_metrics["avg_steps"], total_env_steps)
