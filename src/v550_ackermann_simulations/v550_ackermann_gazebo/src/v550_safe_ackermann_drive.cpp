...
public:
  ...
  // Rear wheel velocity motor torque limits in N*m.
  double drive_torque_limit_{0.08};
  double brake_torque_limit_{0.12};
  double steering_torque_limit_{0.35};

  // Stop stale commands and suppress contact-solver creep while parked.
  double command_timeout_{0.5};
  double parking_linear_threshold_{0.10};
  double parking_angular_threshold_{0.50};
  bool parking_brake_{true};
  bool received_command_{false};
  bool parking_pose_valid_{false};
  gazebo::common::Time last_cmd_time_;
  ignition::math::Pose3d parking_pose_;
  ...
  static double PositiveFiniteOr(double value, double fallback)
  {
    return std::isfinite(value) && value > 0.0 ? value : fallback;
  }

  static double ClampFinite(double value, double lower, double upper)
  {
    if (!std::isfinite(value)) {
      return 0.0;
    }
    return ignition::math::clamp(value, lower, upper);
  }

  void SetRearWheelVelocity(double velocity, double effort)
  {
    effort = PositiveFiniteOr(effort, 0.08);
    velocity = FiniteOr(velocity, 0.0);
    joints_[REAR_RIGHT]->SetParam("fmax", 0, effort);
    joints_[REAR_LEFT]->SetParam("fmax", 0, effort);
    joints_[REAR_RIGHT]->SetParam("vel", 0, velocity);
    joints_[REAR_LEFT]->SetParam("vel", 0, velocity);
  }
};
...
void GazeboRosAckermannDrive::Load(gazebo::physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  impl_->drive_torque_limit_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("drive_torque_limit", 0.08).first, 0.08);
  impl_->brake_torque_limit_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("brake_torque_limit", 0.12).first, impl_->drive_torque_limit_);
  impl_->steering_torque_limit_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("steering_torque_limit", 0.35).first, 0.35);
  impl_->command_timeout_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("command_timeout", 0.5).first, 0.5);
  impl_->parking_linear_threshold_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("parking_linear_threshold", 0.10).first, 0.10);
  impl_->parking_angular_threshold_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    _sdf->Get<double>("parking_angular_threshold", 0.50).first, 0.50);
  impl_->parking_brake_ = _sdf->Get<bool>("parking_brake", true).first;
  ...
  unsigned int id = 0;
  auto fallback_wheel_radius = _sdf->Get<double>("wheel_radius", 0.0332).first;
  ...
  if (!std::isfinite(impl_->wheel_radius_) || impl_->wheel_radius_ <= 1.0e-6) {
    impl_->wheel_radius_ = fallback_wheel_radius;
    RCLCPP_WARN(
      impl_->ros_node_->get_logger(),
      "Rear wheel collision radius was invalid. Falling back to wheel_radius %.4f m.",
      impl_->wheel_radius_);
  }
  ...
  auto fallback_wheel_separation = _sdf->Get<double>("wheel_separation", 0.214).first;
  impl_->wheel_separation_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    distance.Length(), fallback_wheel_separation);
  ...
  auto fallback_wheel_base = _sdf->Get<double>("wheel_base", 0.1432).first;
  impl_->wheel_base_ = GazeboRosAckermannDrivePrivate::PositiveFiniteOr(
    distance.Length(), fallback_wheel_base);

  if (!std::isfinite(impl_->wheel_separation_) || impl_->wheel_separation_ <= 1.0e-6) {
    impl_->wheel_separation_ = 0.214;
  }
  if (!std::isfinite(impl_->wheel_base_) || impl_->wheel_base_ <= 1.0e-6) {
    impl_->wheel_base_ = 0.1432;
  }

  RCLCPP_INFO(
    impl_->ros_node_->get_logger(),
    "V550 drive geometry: wheel_radius=%.4f m, wheel_base=%.4f m, wheel_separation=%.4f m",
    impl_->wheel_radius_, impl_->wheel_base_, impl_->wheel_separation_);

  impl_->SetRearWheelVelocity(0.0, impl_->brake_torque_limit_);
  ...
  impl_->last_cmd_time_ = impl_->last_update_time_;
...
void GazeboRosAckermannDrive::Reset()
{
  ...
  impl_->target_rot_ = 0;
  impl_->received_command_ = false;
  impl_->parking_pose_valid_ = false;
  impl_->SetRearWheelVelocity(0.0, impl_->brake_torque_limit_);
  impl_->pid_left_steering_.Reset();
  impl_->pid_right_steering_.Reset();
...
void GazeboRosAckermannDrivePrivate::OnUpdate(const gazebo::common::UpdateInfo & _info)
{
  ...
  if (!std::isfinite(seconds_since_last_update) || seconds_since_last_update <= 0.0) {
    last_update_time_ = _info.simTime;
    return;
  }

  auto command_age = (_info.simTime - last_cmd_time_).Double();
  bool command_fresh = received_command_ && std::isfinite(command_age) &&
    command_age >= 0.0 && command_age <= command_timeout_;
  auto requested_linear = command_fresh ? target_linear_ : 0.0;
  auto requested_rot = command_fresh ? target_rot_ : 0.0;
  auto target_linear = ignition::math::clamp(
    FiniteOr(requested_linear, 0.0), -max_speed_, max_speed_);
  bool hold_stop = std::abs(target_linear) < 1.0e-4;

  // Run the parking brake at the physics rate, not only at the controller update rate.
  // Once the chassis has slowed, pin its settled pose to eliminate contact-solver creep.
  if (hold_stop && parking_brake_) {
    SetRearWheelVelocity(0.0, brake_torque_limit_);
    auto linear_velocity = model_->WorldLinearVel();
    auto angular_velocity = model_->WorldAngularVel();
    auto current_pose = model_->WorldPose();
    if (parking_pose_valid_) {
      auto position_delta = current_pose.Pos() - parking_pose_.Pos();
      auto yaw_delta = std::abs(std::atan2(
          std::sin(current_pose.Rot().Yaw() - parking_pose_.Rot().Yaw()),
          std::cos(current_pose.Rot().Yaw() - parking_pose_.Rot().Yaw())));
      if (std::hypot(position_delta.X(), position_delta.Y()) > 0.01 || yaw_delta > 0.02) {
        // Accept intentional SetEntityState teleports instead of snapping back to the old parking pose.
        parking_pose_ = current_pose;
      }
    }
    if (!parking_pose_valid_ &&
      linear_velocity.Length() < parking_linear_threshold_ &&
      angular_velocity.Length() < parking_angular_threshold_)
    {
      parking_pose_ = current_pose;
      parking_pose_valid_ = true;
    }
    if (parking_pose_valid_) {
      model_->SetWorldPose(parking_pose_);
      model_->SetLinearVel(ignition::math::Vector3d::Zero);
      model_->SetAngularVel(ignition::math::Vector3d::Zero);
    }
  } else {
    parking_pose_valid_ = false;
  }
...
#endif
  double target_wheel_velocity = target_linear / wheel_radius_;
  target_wheel_velocity = ignition::math::clamp(target_wheel_velocity, -45.0, 45.0);
  if (hold_stop) {
    target_wheel_velocity = 0.0;
  }
  SetRearWheelVelocity(
    target_wheel_velocity, hold_stop ? brake_torque_limit_ : drive_torque_limit_);

  auto target_rot = -FiniteOr(requested_rot, 0.0) * copysign(1.0, target_linear);
  ...
  auto left_steering_angle = FiniteOr(joints_[STEER_LEFT]->Position(0), 0.0);
  auto right_steering_angle = FiniteOr(joints_[STEER_RIGHT]->Position(0), 0.0);
  ...
  left_steering_cmd = ClampFinite(
    left_steering_cmd, -steering_torque_limit_, steering_torque_limit_);
  ...
  right_steering_cmd = ClampFinite(
    right_steering_cmd, -steering_torque_limit_, steering_torque_limit_);
...
void GazeboRosAckermannDrivePrivate::OnCmdVel(const geometry_msgs::msg::Twist::SharedPtr _msg)
{
  ...
  target_linear_ = FiniteOr(_msg->linear.x, 0.0);
  target_rot_ = FiniteOr(_msg->angular.z, 0.0);
...
void GazeboRosAckermannDrivePrivate::UpdateOdometryWorld()
{
  ...
  odom_.twist.twist.angular.z = FiniteOr(angular.Z(), 0.0);

  // Convert velocity to child_frame_id(aka base_footprint)
  auto yaw = static_cast<float>(pose.Rot().Yaw());
  auto linear_x = FiniteOr(linear.X(), 0.0);
  auto linear_y = FiniteOr(linear.Y(), 0.0);
  odom_.twist.twist.linear.x = cosf(yaw) * linear_x + sinf(yaw) * linear_y;
  odom_.twist.twist.linear.y = cosf(yaw) * linear_y - sinf(yaw) * linear_x;
}
